...
const loadStoredState = (initialUser: UserProfile | null): AppState => {
  ...
  try {
    const saved = window.localStorage.getItem('micro-store-state-v2');
    if (saved) {
      const parsed = JSON.parse(saved);
      // Schema version check — clear stale/corrupt data from old versions
      if (parsed.version !== VERSION) {
        window.localStorage.removeItem('micro-store-state-v2');
        return {
          ...initialState,
          user: initialUser,
        };
...
export const StoreProvider: React.FC<{ children: ReactNode; initialUser: UserProfile | null }> = ({ children, initialUser }) => {
  ...
  React.useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'micro-store-state-v2' && e.newValue) {
        try {
  ...
  React.useEffect(() => {
    ...
    if (window.localStorage.getItem('micro-store-state-v2') !== currentState) {
      window.localStorage.setItem('micro-store-state-v2', currentState);
    }
  }, [hydrated, state]);
