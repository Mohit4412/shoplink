      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Orders</span>
          </div>
        ...
        <div className="rounded-xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Revenue</span>
          </div>
        ...
        <div className="rounded-xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Customers</span>
          </div>
          <p className="text-xl font-black text-gray-900">{uniqueCustomers}</p>
        </div>
      </div>
        ...
        {paginated.map((order) => {
          ...
          return (
            <button
              ...
              className="w-full rounded-xl border border-gray-200 bg-white overflow-hidden text-left shadow-sm hover:border-gray-300 transition-colors"
            >
              {/* Card header */}
              <div className="flex items-center gap-3 px-3 py-2.5 border-b border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400">{lead.details.customerPhone || 'No phone'}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-emerald-700">{currencySymbol}{order.revenue.toFixed(2)}</p>
                  <p className="text-[11px] text-gray-400">× {order.quantity}</p>
                </div>
              </div>
              {/* Card body */}
              <div className="flex items-center gap-3 px-3 py-2">
                {product?.imageUrl ? (
                  <img src={product.imageUrl} alt={product.name} className="w-9 h-9 rounded-lg object-cover border border-gray-100 shrink-0" />
                ) : (
                  <div className="w-9 h-9 rounded-lg bg-gray-100 shrink-0" />
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-gray-700 truncate">{product?.name || 'Unknown product'}</p>
                  <p className="text-[11px] text-gray-400">{formatPaymentMethodLabel(lead.details.paymentMethod) || 'Payment N/A'}</p>
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  <p className="text-[11px] text-gray-400">{format(new Date(order.date), 'MMM d, h:mm a')}</p>
                </div>
              </div>
            </button>
          );
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Customer</th>
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Product</th>
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Contact</th>
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Payment</th>
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Shipping</th>
              <th className="px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-gray-500">Qty</th>
              <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500">Status</th>
              <th className="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-gray-500">Revenue</th>
              <th className="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-gray-500">Date</th>
            </tr>
          </thead>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate max-w-[120px]">{customerName}</p>
                      </div>
                    </div>
                  </td>
                  {/* Product */}
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {product?.imageUrl && (
                        <img src={product.imageUrl} alt={product.name} className="w-8 h-8 rounded-lg object-cover border border-gray-100 shrink-0" />
                      )}
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    <div>
                      <p className="whitespace-nowrap">{lead.details.customerPhone || <span className="text-gray-300">—</span>}</p>
                      {lead.details.email && (
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                    {formatPaymentMethodLabel(lead.details.paymentMethod) || <span className="text-gray-300">—</span>}
                  </td>
                  {/* Shipping */}
                  ...
                  {/* Qty */}
                  <td className="px-4 py-3 text-center text-sm font-medium text-gray-700">{order.quantity}</td>
                  {/* Status */}
                  <td className="px-4 py-3">
                    <StatusBadge status={order.status} />
                  </td>
