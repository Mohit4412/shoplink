...
export function Products() {
  ...
  const [mobileColCleared, setMobileColCleared] = useState(false);
  const [collectionProductsPage, setCollectionProductsPage] = useState(1);

  useEffect(() => {
    if (activeTab === 'collections' && collections.length > 0) {
      // Only auto-select if user hasn't explicitly cleared the selection on mobile
      if (!mobileColCleared && (!selectedCol || !collections.find(c => c.name === selectedCol))) {
        setSelectedCol(collections[0].name);
      }
    }
    // Reset the cleared flag when switching away from collections tab
    if (activeTab !== 'collections') {
      setMobileColCleared(false);
    }
          ...
          <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3.5">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">{collections.length} collection{collections.length !== 1 ? 's' : ''} · {products.filter((p) => getProductCollections(p).length > 0).length} assigned</p>
              </div>
              <button
                onClick={promptNewCollection}
                className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-zinc-900 px-3.5 text-sm font-semibold text-white transition-colors hover:bg-zinc-700 shrink-0"
              >
            ...
            {collections.length === 0 ? (
              <div className="px-6 py-16 text-center">
                ...
                <h3 className="mt-4 text-base font-semibold text-zinc-900">No collections yet</h3>
                <p className="mx-auto mt-2 max-w-xs text-sm text-zinc-500">
                  Create collections for categories like New Arrivals or Best Sellers.
                </p>
              </div>
            ) : (
              <>
                  ...
                  {!selectedCol ? (
                    <div className="divide-y divide-zinc-100">
                      {collections.map((col) => (
                        <button
                          ...
                          className="flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left hover:bg-zinc-50 transition-colors"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-zinc-900 truncate">{col.name}</p>
                              <p className="text-xs text-zinc-400">{col.count} {col.count === 1 ? 'product' : 'products'}</p>
                            </div>
                          </div>
                    ...
                    <div>
                      <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3">
                        <button
                          type="button"
                          onClick={() => { setSelectedCol(null); setMobileColCleared(true); }}
                        ...
                        <p className="text-sm font-semibold text-zinc-900 truncate">{selectedCol}</p>
                      </div>
                      <div className="flex flex-wrap gap-2 px-4 py-3 border-b border-zinc-100">
                        <button onClick={() => setAttachProductsModalOpen(true)}
                          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-zinc-200 px-3 text-xs font-medium text-zinc-700 hover:bg-zinc-50 transition-colors">
                          <Package className="h-3.5 w-3.5" /> Add Existing
                        </button>
                        <button onClick={() => { resetForm(); setFormData(prev => ({ ...prev, collection: selectedCol })); setIsAddModalOpen(true); }}
                          className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-zinc-900 px-3 text-xs font-semibold text-white hover:bg-zinc-700 transition-colors">
                          <Plus className="h-3.5 w-3.5" /> Add Product
                        </button>
                        <button onClick={() => handleRenameCollection(selectedCol)}
                          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-zinc-200 px-3 text-xs font-medium text-zinc-700 hover:bg-zinc-50 transition-colors">
                          <Edit2 className="h-3.5 w-3.5" /> Rename
                        </button>
                        <button onClick={() => handleDeleteCollection(selectedCol)}
                          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-red-200 px-3 text-xs font-medium text-red-600 hover:bg-red-50 transition-colors">
                          <Trash2 className="h-3.5 w-3.5" /> Delete
                        </button>
                      </div>
                      <div className="grid grid-cols-3 divide-x divide-zinc-100 border-b border-zinc-100">
                        {[
                          { label: 'Total', value: selectedCollectionProducts.length },
                          { label: 'Active', value: selectedCollectionProducts.filter((p) => p.status === 'Active').length },
                          { label: 'Inactive', value: selectedCollectionProducts.filter((p) => p.status !== 'Active').length },
                        ].map(({ label, value }) => (
                      ...
                      {selectedCollectionProducts.length === 0 ? (
                        <div className="px-5 py-10 text-center">
                          ...
                          <p className="mt-2 text-sm font-medium text-zinc-600">No products yet</p>
                        </div>
                      ) : (
                        <div className="divide-y divide-zinc-100">
                          {paginatedCollectionProducts.map((p) => (
                            <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                              <img src={p.imageUrl} alt={p.name} className="h-12 w-12 rounded-lg object-cover border border-zinc-100 shrink-0" />
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-zinc-900 truncate">{p.name}</p>
                                <p className="text-xs text-zinc-400">{currencySymbol}{p.price.toFixed(2)}</p>
                              </div>
                              <span className={`shrink-0 rounded-md px-2 py-0.5 text-[11px] font-semibold border ${p.status === 'Active' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-zinc-100 border-zinc-200 text-zinc-500'}`}>{p.status}</span>
                              <button onClick={() => handleRemoveFromCollection(p.id)}
                                className="shrink-0 flex h-8 w-8 items-center justify-center rounded-lg border border-red-200 text-red-500 hover:bg-red-50 transition-colors">
                                <Trash2 className="h-3.5 w-3.5" />
                              </button>
                            </div>
                          ))}
                          {collectionProductsTotalPages > 1 && (
                            <div className="flex items-center justify-between px-4 py-3 bg-zinc-50">
                              <span className="text-xs text-zinc-400">
                                {(currentCollectionProductsPage - 1) * COLLECTION_PRODUCTS_PAGE_SIZE + 1}–{Math.min(currentCollectionProductsPage * COLLECTION_PRODUCTS_PAGE_SIZE, selectedCollectionProducts.length)} of {selectedCollectionProducts.length}
                              </span>
                              <div className="flex gap-1">
                                <button type="button" onClick={() => setCollectionProductsPage((page) => Math.max(1, page - 1))} disabled={currentCollectionProductsPage === 1}
                                  className="h-8 w-8 flex items-center justify-center rounded-lg border border-zinc-200 bg-white disabled:opacity-40">
                                  <ChevronLeft className="h-4 w-4" />
                                </button>
                                <button type="button" onClick={() => setCollectionProductsPage((page) => Math.min(collectionProductsTotalPages, page + 1))} disabled={currentCollectionProductsPage === collectionProductsTotalPages}
                                  className="h-8 w-8 flex items-center justify-center rounded-lg border border-zinc-200 bg-white disabled:opacity-40">
                ...
                <div className="hidden md:grid md:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
                  <div className="border-r border-zinc-100">
                    <div className="px-5 py-3 text-xs font-semibold uppercase tracking-widest text-zinc-400">All Collections</div>
                    <div className="divide-y divide-zinc-100">
                      {collections.map((col) => {
                        const isActive = selectedCol === col.name;
                        return (
                          <button key={col.name} onClick={() => setSelectedCol(col.name)}
                            className={`flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition-colors ${isActive ? 'bg-zinc-50' : 'bg-white hover:bg-zinc-50'}`}>
                            <div className="min-w-0">
                              <p className="truncate text-sm font-semibold text-zinc-900">{col.name}</p>
                              <p className="mt-1 text-xs text-zinc-500">{col.count} {col.count === 1 ? 'product' : 'products'}</p>
                            </div>
                            <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${isActive ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-600'}`}>{col.count}</span>
                          </button>
                        );
                      })}
                    </div>
                    ...
                    {selectedCol ? (
                      <>
                        <div className="flex flex-col gap-4 border-b border-zinc-100 pb-4 xl:flex-row xl:items-start xl:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <button onClick={() => setAttachProductsModalOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-zinc-200 px-3.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50"><Package className="h-4 w-4" /> Add Existing</button>
                            <button onClick={() => { resetForm(); setFormData(prev => ({ ...prev, collection: selectedCol })); setIsAddModalOpen(true); }} className="inline-flex h-9 items-center gap-2 rounded-lg bg-zinc-900 px-3.5 text-sm font-semibold text-white transition-colors hover:bg-zinc-700"><Plus className="h-4 w-4" /> Add Product</button>
                            <button onClick={() => handleRenameCollection(selectedCol)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-zinc-200 px-3.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50"><Edit2 className="h-4 w-4" /> Rename</button>
                            <button onClick={() => handleDeleteCollection(selectedCol)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-red-200 px-3.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"><Trash2 className="h-4 w-4" /> Delete</button>
                          </div>
                        </div>
                        <div className="mt-4 grid gap-3 md:grid-cols-3">
                          {[{ label: 'Total Products', value: selectedCollectionProducts.length }, { label: 'Active', value: selectedCollectionProducts.filter((p) => p.status === 'Active').length }, { label: 'Inactive', value: selectedCollectionProducts.filter((p) => p.status !== 'Active').length }].map(({ label, value }) => (
                            ...
                            <div className="flex h-[420px] flex-col">
                              <div className="flex-1 overflow-x-auto overflow-y-auto">
                                <table className="min-w-[720px] w-full text-sm">
                                  <thead className="bg-zinc-50"><tr className="border-b border-zinc-200">
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Product</th>
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Status</th>
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Price</th>
                                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Action</th>
                                  </tr></thead>
                                  <tbody className="divide-y divide-zinc-100 bg-white">
                                    {paginatedCollectionProducts.map((p) => (
                                      <tr key={p.id} className="hover:bg-zinc-50">
                                        <td className="px-4 py-3"><div className="flex items-center gap-3 min-w-0"><img src={p.imageUrl} alt={p.name} className="h-12 w-12 rounded-xl border border-zinc-100 object-cover" /><div className="min-w-0"><p className="truncate font-medium text-zinc-900">{p.name}</p><p className="mt-0.5 line-clamp-1 text-xs text-zinc-500">{p.description || 'No description'}</p></div></div></td>
                                        <td className="px-4 py-3"><span className={`inline-flex rounded-md px-2.5 py-1 text-[11px] font-semibold border ${p.status === 'Active' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-zinc-100 border-zinc-200 text-zinc-600'}`}>{p.status}</span></td>
                                        <td className="px-4 py-3 font-medium text-zinc-700">{currencySymbol}{p.price.toFixed(2)}</td>
                                        <td className="px-4 py-3 text-right"><button onClick={() => handleRemoveFromCollection(p.id)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-red-200 px-3 text-xs font-medium text-red-600 transition-colors hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /> Remove</button></td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
                              ...
                              <div className="mt-auto flex items-center justify-between border-t border-zinc-200 bg-zinc-50 px-4 py-3">
                                <span className="text-xs text-zinc-500">{(currentCollectionProductsPage - 1) * COLLECTION_PRODUCTS_PAGE_SIZE + 1}–{Math.min(currentCollectionProductsPage * COLLECTION_PRODUCTS_PAGE_SIZE, selectedCollectionProducts.length)} of {selectedCollectionProducts.length}</span>
                                <div className="flex items-center gap-1">
                                  <button type="button" onClick={() => setCollectionProductsPage((page) => Math.max(1, page - 1))} disabled={currentCollectionProductsPage === 1} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-zinc-100 disabled:opacity-40"><ChevronLeft className="h-4 w-4" /></button>
                                  <button type="button" onClick={() => setCollectionProductsPage((page) => Math.min(collectionProductsTotalPages, page + 1))} disabled={currentCollectionProductsPage === collectionProductsTotalPages} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-zinc-100 disabled:opacity-40"><ChevronRight className="h-4 w-4" /></button>
                                </div>
                              </div>
                            </div>
                          )}
                      ...
                      <div className="flex min-h-[300px] items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50 text-center">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-zinc-500">Choose from the left to manage it.</p>
                        </div>
                      </div>
                    )}
                  </div>
